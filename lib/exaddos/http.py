		...
		for d in data[t]:
			...
			for key in info:
				...
				if number >= maximum[0]:
					...
					best.setdefault(number,[]).append(ip)
					maximum = sorted(maximum[1:]+[number,])

	r = []
	# XXXX: BIG harcoded max here with 5
	for number in reversed(maximum):
		for ip in best[number]:
			r.append({'ip': ip, 'value': number})

	return json.dumps(r[:nb_keeping])
